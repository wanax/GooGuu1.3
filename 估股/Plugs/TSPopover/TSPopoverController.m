...
- (id)init {
	if ((self = [super init])) {
        ...
        self.titleFont = [UIFont boldSystemFontOfSize:1];
...
- (void) showPopoverWithTouch:(UIEvent*)senderEvent andSuperView:(UIView *)superView
{    
    ...
    [self showPopoverWithPoint:senderPoint andSuperView:superView];
}
...
- (void) showPopoverWithCell:(UITableViewCell*)senderCell
{
    ...
    CGPoint senderPoint = [self senderPointFromSenderRect:senderFrame];
    [self showPopoverWithPoint:senderPoint andSuperView:nil];
}
...
- (void) showPopoverWithPoint:(CGPoint)senderPoint andSuperView:(UIView *)superView
{
    if(self.titleText){
